...
class ListDatabaseRolesAsyncPager:
    ...
    """
    def __init__(self,
            method: Callable[..., Awaitable[spanner_database_admin.ListDatabaseRolesResponse]],
            request: spanner_database_admin.ListDatabaseRolesRequest,
            response: spanner_database_admin.ListDatabaseRolesResponse,
            *,
            retry: OptionalAsyncRetry = gapic_v1.method.DEFAULT,
    ...
    @property
    async def pages(self) -> AsyncIterator[spanner_database_admin.ListDatabaseRolesResponse]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = await self._method(self._request, retry=self._retry, timeout=self._timeout, metadata=self._metadata)
