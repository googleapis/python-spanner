...
class Database(object):
    ...
    * :meth:`drop` the database
            ...
            with trace_call(
                ...
                try:
                    ...
                    with error_augmenter:
                        txn = await api.begin_transaction(
    ...
    async def restore(self, source):
        ...
        future = await api.restore_database(
            request=request,
            metadata=self.metadata_with_request_id(self._next_nth_request, 1, metadata),
        )
        return future
...
class BatchCheckout(object):
    ...
    async def __aenter__(self):
        ...
        transaction_type = TransactionType.READ_ONLY
        self._session = await self._database.sessions_manager.get_session(
            transaction_type
        )
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            ...
            await self._database.sessions_manager.put_session(self._session)
...
class MutationGroupsCheckout(object):
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if isinstance(exc_val, NotFound):
            ...
            if not await self._session.exists():
...
class BatchSnapshot(object):
    ...
    async def _get_session(self):
        ...
        if self._session is None:
            ...
            if self._session_id is None:
                transaction_type = TransactionType.PARTITIONED
                session = await database.sessions_manager.get_session(transaction_type)
    ...
    async def read(self, *args, **kw):
        ...
        """
        snapshot = await self._get_snapshot()
        return await CrossSync.run_if_async(snapshot.read, *args, **kw)

    @CrossSync.convert
    async def execute_sql(self, *args, **kw):
        """Convenience method:  perform query operation via snapshot.

        See :meth:`~google.cloud.spanner_v1.snapshot.Snapshot.execute_sql`.
        """
        snapshot = await self._get_snapshot()
        return await CrossSync.run_if_async(snapshot.execute_sql, *args, **kw)

    @CrossSync.convert
    async def generate_read_batches(
        self,
        table,
        columns,
        keyset,
    ...
    @CrossSync.convert
    async def run_partitioned_query(
        self,
        sql,
        params=None,
        param_types=None,
    ...
    async def process(self, batch):
        ...
        if "query" in batch:
            return await self.process_query_batch(batch)
        if "read" in batch:
            return await self.process_read_batch(batch)
