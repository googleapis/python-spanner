...
class TestSpannerClient(object):
    ...
    def test_batch_create_sessions(self):
        ...
        assert len(channel.requests) == 1
        expected_request = spanner_pb2.BatchCreateSessionsRequest(database=database, session_count=session_count)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_execute_batch_dml(self):
        ...
        assert len(channel.requests) == 1
        expected_request = spanner_pb2.ExecuteBatchDmlRequest(session=session, transaction=transaction, statements=statements, seqno=seqno)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_read(self):
        ...
        assert len(channel.requests) == 1
        expected_request = spanner_pb2.ReadRequest(session=session, table=table, columns=columns, key_set=key_set)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_rollback(self):
        ...
        assert len(channel.requests) == 1
        expected_request = spanner_pb2.RollbackRequest(session=session, transaction_id=transaction_id)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_partition_read(self):
        ...
        assert len(channel.requests) == 1
        expected_request = spanner_pb2.PartitionReadRequest(session=session, table=table, key_set=key_set)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
