...
async def test_list_instance_configs_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page in (await client.list_instance_configs(request={})).pages:
            pages.append(page)
        for page, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page.raw_page.next_page_token == token
...
def test_instance_admin_grpc_transport_channel_mtls_with_client_cert_source(
...
def test_instance_admin_grpc_asyncio_transport_channel_mtls_with_client_cert_source(
    grpc_create_channel, grpc_ssl_channel_cred
):
    # Check that if channel is None, but api_mtls_endpoint and client_cert_source
    # are provided, then a mTLS channel will be created.
...
@mock.patch("google.api_core.grpc_helpers.create_channel", autospec=True)
def test_instance_admin_grpc_transport_channel_mtls_with_adc(
...
@mock.patch("google.api_core.grpc_helpers_async.create_channel", autospec=True)
def test_instance_admin_grpc_asyncio_transport_channel_mtls_with_adc(
    grpc_create_channel, api_mtls_endpoint
):
    # Check that if channel and client_cert_source are None, but api_mtls_endpoint
    # is provided, then a mTLS channel will be created with SSL ADC.
