...
from google.cloud.spanner_v1.types import ClientContext, RequestOptions
from google.cloud.spanner_v1._helpers import _merge_client_context, _merge_request_options
...
class TestClientContext(unittest.TestCase):
    ...
    def test__merge_client_context_both_set(self):
        base = ClientContext(secure_context={
            "a": struct_pb2.Value(string_value="A"),
            "b": struct_pb2.Value(string_value="B1")
        })
        merge = ClientContext(secure_context={
    ...
    def test__merge_request_options_with_client_context(self):
        ...
        client_context = ClientContext(secure_context={"a": struct_pb2.Value(string_value="A")})
    ...
    def test_client_init_with_client_context(self):
        ...
        project = "PROJECT"
        with mock.patch("google.cloud.spanner_v1.client._get_spanner_enable_builtin_metrics_env", return_value=False):
            client_context = {"secure_context": {"a": struct_pb2.Value(string_value="A")}}
            client = Client(project=project, client_context=client_context, disable_builtin_metrics=True)
    ...
    def test_snapshot_execute_sql_propagates_client_context(self):
        ...
        client._client_context = ClientContext(secure_context={"client": struct_pb2.Value(string_value="from-client")})
        
        snapshot_context = ClientContext(secure_context={"snapshot": struct_pb2.Value(string_value="from-snapshot")})
        ...
        with mock.patch.object(snapshot, "_get_streamed_result_set") as mocked:
            ...
            self.assertEqual(request.request_options.client_context.secure_context["client"], "from-client")
            self.assertEqual(request.request_options.client_context.secure_context["snapshot"], "from-snapshot")

    def test_transaction_commit_propagates_client_context(self):
        from google.cloud.spanner_v1.transaction import Transaction
        from google.cloud.spanner_v1.types import CommitRequest, CommitResponse, MultiplexedSessionPrecommitToken
        ...
        transaction_context = ClientContext(secure_context={"txn": struct_pb2.Value(string_value="from-txn")})
        ...
        with mock.patch("google.cloud.spanner_v1.transaction._retry", side_effect=side_effect):
            transaction.commit()
        ...
        self.assertEqual(request.request_options.client_context.secure_context["client"], "from-client")
        self.assertEqual(request.request_options.client_context.secure_context["txn"], "from-txn")
    ...
    def test_snapshot_execute_sql_request_level_override(self):
        ...
        client = database._instance._client = mock.Mock()
        client._query_options = None
        client._client_context = ClientContext(secure_context={"a": struct_pb2.Value(string_value="from-client")})
        
        snapshot_context = ClientContext(secure_context={"a": struct_pb2.Value(string_value="from-snapshot"), "b": struct_pb2.Value(string_value="B")})
        snapshot = Snapshot(session, client_context=snapshot_context)
        
        request_options = RequestOptions(client_context=ClientContext(secure_context={"a": struct_pb2.Value(string_value="from-request")}))
        
        with mock.patch.object(snapshot, "_get_streamed_result_set") as mocked:
            snapshot.execute_sql("SELECT 1", request_options=request_options)
            kwargs = mocked.call_args.kwargs
            request = kwargs['request']
            self.assertEqual(request.request_options.client_context.secure_context["a"], "from-request")
            self.assertEqual(request.request_options.client_context.secure_context["b"], "B")

    def test_batch_commit_propagates_client_context(self):
        from google.cloud.spanner_v1.batch import Batch
        from google.cloud.spanner_v1.types import CommitRequest, CommitResponse, TransactionOptions
        ...
        batch_context = ClientContext(secure_context={"batch": struct_pb2.Value(string_value="from-batch")})
        ...
        self.assertIsInstance(request, CommitRequest)
        self.assertEqual(request.request_options.client_context.secure_context["client"], "from-client")
        self.assertEqual(request.request_options.client_context.secure_context["batch"], "from-batch")

    def test_transaction_execute_update_propagates_client_context(self):
        from google.cloud.spanner_v1.transaction import Transaction
        from google.cloud.spanner_v1.types import ExecuteSqlRequest, ResultSet, MultiplexedSessionPrecommitToken
        ...
        api = database.spanner_api = mock.Mock()
        response = ResultSet(precommit_token=MultiplexedSessionPrecommitToken(seq_num=2))
    ...
    def test_mutation_groups_batch_write_propagates_client_context(self):
        ...
        mg_context = ClientContext(secure_context={"mg": struct_pb2.Value(string_value="from-mg")})
        mg = MutationGroups(session, client_context=mg_context)
        
        api = database.spanner_api = mock.Mock()
        
        with mock.patch("google.cloud.spanner_v1.batch._retry", side_effect=lambda f, **kw: f()):
            mg.batch_write()
        ...
        self.assertEqual(request.request_options.client_context.secure_context["client"], "from-client")
        self.assertEqual(request.request_options.client_context.secure_context["mg"], "from-mg")
    ...
    def test_database_snapshot_propagates_client_context(self):
        ...
        with mock.patch("google.cloud.spanner_v1.database.SnapshotCheckout") as mocked_checkout:
            client_context = {"secure_context": {"a": struct_pb2.Value(string_value="A")}}
            database.snapshot(client_context=client_context)
            
            mocked_checkout.assert_called_once_with(database, client_context=client_context)
