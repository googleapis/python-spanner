...
class TestSession(OpenTelemetryBase):
    ...
    @CrossSync.pytest
    async def test_create_w_database_role(self, mock_region):
    ...
    @CrossSync.pytest
    async def test_create_session_span_annotations(self, mock_region):
        session_pb = self._make_session_pb(
            self.SESSION_NAME, database_role=self.DATABASE_ROLE
        )
    ...
    @CrossSync.pytest
    async def test_create_wo_database_role(self, mock_region):
    ...
    @CrossSync.pytest
    async def test_create_ok(self, mock_region):
        session_pb = self._make_session_pb(self.SESSION_NAME)
        gax_api = self._make_spanner_api()
        gax_api.create_session.return_value = session_pb
        database = self._make_database()
    ...
    @CrossSync.pytest
    async def test_create_w_labels(self, mock_region):
        labels = {"foo": "bar"}
    ...
    @CrossSync.pytest
    async def test_create_error(self, mock_region):
        gax_api = self._make_spanner_api()
    ...
    async def test_exists_wo_session_id(self):
        database = self._make_database()
        session = self._make_one(database)
        self.assertFalse(await session.exists())
    ...
    @CrossSync.pytest
    async def test_exists_hit(self, mock_region):
        session_pb = self._make_session_pb(self.SESSION_NAME)
        gax_api = self._make_spanner_api()
        ...
        self.assertTrue(await session.exists())
    ...
    @CrossSync.pytest
    async def test_exists_miss(self, mock_region):
        gax_api = self._make_spanner_api()
    ...
    @CrossSync.pytest
    async def test_exists_error(self, mock_region):
        gax_api = self._make_spanner_api()
    ...
    @CrossSync.pytest
    async def test_ping_hit(self, mock_region):
        gax_api = self._make_spanner_api()
    ...
    @CrossSync.pytest
    async def test_ping_miss(self, mock_region):
        gax_api = self._make_spanner_api()
    ...
    @CrossSync.pytest
    async def test_ping_error(self, mock_region):
        gax_api = self._make_spanner_api()
    ...
    @CrossSync.pytest
    async def test_delete_hit(self, mock_region):
        gax_api = self._make_spanner_api()
    ...
    @CrossSync.pytest
    async def test_delete_miss(self, mock_region):
        gax_api = self._make_spanner_api()
    ...
    @CrossSync.pytest
    async def test_delete_error(self, mock_region):
        gax_api = self._make_spanner_api()
    ...
    async def test_snapshot_created(self):
        database = self._make_database()
        session = self._make_one(database)
        session._session_id = "DEADBEEF"  # emulate 'await session.create()'
    ...
    @CrossSync.pytest
    async def test_run_in_transaction_w_abort_w_retry_metadata_w_exclude_txn_from_change_streams(
        self,
    ):
        RETRY_SECONDS = 12
        RETRY_NANOS = 3456
    ...
    async def test_run_in_transaction_w_isolation_level_at_client(self):
        ...
        return_value = await session.run_in_transaction(unit_of_work, "abc")
    ...
    @CrossSync.pytest
    async def test_run_in_transaction_w_isolation_level_and_read_lock_mode_at_request_overrides_client(
        self,
    ):
        database = self._make_database(
            default_transaction_options=DefaultTransactionOptions(
    ...
    async def test_delay_helper_w_no_delay(self):
        ...
        exc_mock = CrossSync.Mock(errors=[metadata_mock])
