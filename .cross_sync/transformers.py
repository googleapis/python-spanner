...
sys.path.append(os.path.join(repo_root, "google", "cloud", "aio", "_cross_sync"))
...
class AsyncToSync(ast.NodeTransformer):
    ...
    def visit_AsyncFunctionDef(self, node):
        ...
        """
        if node.name in ("__anext__", "__aiter__", "__aenter__", "__aexit__"):
            node.name = node.name.replace("__a", "__")
...
class CrossSyncFileProcessor(ast.NodeTransformer):
    ...
    def visit_Module(self, node):
        ...
        if output_path:
            ...
            converted = SymbolReplacer(replacements).visit(converted)
            return converted
        else:
